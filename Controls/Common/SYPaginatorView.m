...
- (id)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		_pageControl = [[SYPageControl alloc] initWithFrame:CGRectMake(0.0f, -100.0f, size.width, 18.0f)];
...
- (void)_loadPage:(NSInteger)page {
    ...
    //remove offline pages to release memory pressure
    if (_currentPageIndex>=0)
    {
        ...
        [_pages enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            ...
            if (abs(intKey - _currentPageIndex)>offlineGap)
               [removeKeys addObject:key];
                
        }];
        [removeKeys enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            [[self pageForIndex:[obj intValue]] willRemoveFromSuper];
            [[self pageForIndex:[obj intValue]] removeFromSuperview];
            [_pages removeObjectForKey:obj];
            NSLog(@"page index removed:%d",[obj intValue]);
        }];
    }
