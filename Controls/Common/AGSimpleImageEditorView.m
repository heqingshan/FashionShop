...
- (CGRect)imageFrameFromImageViewWithAspectFitMode:(UIImageView *)theImageView
{
    ...
    _rect.size.height = APP_HIGH - 44;
    CGSize imageSize = CGSizeAbsolute([self sizeForRotatedImage:self.imageView.image]);
    float imageRatio = imageSize.width / imageSize.height;
    float viewRatio = _rect.size.width / _rect.size.height;
    
    if (imageRatio < viewRatio)
    {
        float scale = _rect.size.height / imageSize.height; 
        float width = scale * imageSize.width;
        float topLeftX = .5 * (_rect.size.width - width);
        return CGRectMake(topLeftX, 0, width, _rect.size.height);
    }
    else
    {
        float scale = _rect.size.width / imageSize.width;
        float height = scale * imageSize.height;
        float topLeftY = .5 * (_rect.size.height - height);
        return CGRectMake(0, topLeftY, _rect.size.width, height);
    }
}
