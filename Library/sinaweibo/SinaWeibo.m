    ...
    else
    {
        ...
        */
        if (!ssoLoggingIn)
        {
             ...
             */
            SinaWeiboAuthorizeViewController *authorizeController = [[SinaWeiboAuthorizeViewController alloc] initWithAuthParams:params delegate:self];
            UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:authorizeController];
            navController.navigationBar.barStyle = UIBarStyleBlackOpaque;
            if ([delegate isKindOfClass:[UIViewController class]])
            {
                [((UIViewController *)delegate)  presentViewController:navController animated:true completion:nil];
            }
...
- (void)authorizeViewDidCancel:(SinaWeiboAuthorizeViewController *)authView
{
    [self logInDidCancel];
}

#pragma mark - SinaWeiboAuthorizeViewController Delegate

- (void)authorizeView2:(SinaWeiboAuthorizeViewController *)authView didRecieveAuthorizationCode:(NSString *)code
{
    [self requestAccessTokenWithAuthorizationCode:code];
}

- (void)authorizeView2:(SinaWeiboAuthorizeViewController *)authView didFailWithErrorInfo:(NSDictionary *)errorInfo
{
    [self logInDidFailWithErrorInfo:errorInfo];
}
